// Name: Sport Scramble, Version: 1

#include "../pch.h"

/*!!DEFINE!!*/

/*!!HELPER_DEF!!*/

/*!!HELPER_INC!!*/

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
//---------------------------------------------------------------------------
// Functions
//---------------------------------------------------------------------------

// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.AddActorToGameElementList
// (Public, HasOutParms, BlueprintCallable, BlueprintEvent)
// Parameters:
// class AActor*                  Actor                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// bool                           Callback                       (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor)
void ATN_Minigame_AiTest_C::AddActorToGameElementList(class AActor* Actor, bool* Callback)
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.AddActorToGameElementList");

	ATN_Minigame_AiTest_C_AddActorToGameElementList_Params params;
	params.Actor = Actor;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

	if (Callback != nullptr)
		*Callback = params.Callback;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.Shutdown
// (Public, HasOutParms, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                           ShutdownBegun                  (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor)
void ATN_Minigame_AiTest_C::Shutdown(bool* ShutdownBegun)
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.Shutdown");

	ATN_Minigame_AiTest_C_Shutdown_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

	if (ShutdownBegun != nullptr)
		*ShutdownBegun = params.ShutdownBegun;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.BindToManager
// (Public, BlueprintCallable, BlueprintEvent)
void ATN_Minigame_AiTest_C::BindToManager()
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.BindToManager");

	ATN_Minigame_AiTest_C_BindToManager_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.OnRep_Manager
// (BlueprintCallable, BlueprintEvent)
void ATN_Minigame_AiTest_C::OnRep_Manager()
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.OnRep_Manager");

	ATN_Minigame_AiTest_C_OnRep_Manager_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.SpawnGameElements
// (Public, HasDefaults, BlueprintCallable, BlueprintEvent)
void ATN_Minigame_AiTest_C::SpawnGameElements()
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.SpawnGameElements");

	ATN_Minigame_AiTest_C_SpawnGameElements_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.Initialize
// (Public, BlueprintCallable, BlueprintEvent)
void ATN_Minigame_AiTest_C::Initialize()
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.Initialize");

	ATN_Minigame_AiTest_C_Initialize_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.UserConstructionScript
// (Event, Public, BlueprintCallable, BlueprintEvent)
void ATN_Minigame_AiTest_C::UserConstructionScript()
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.UserConstructionScript");

	ATN_Minigame_AiTest_C_UserConstructionScript_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.ReceiveBeginPlay
// (Event, Protected, BlueprintEvent)
void ATN_Minigame_AiTest_C::ReceiveBeginPlay()
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.ReceiveBeginPlay");

	ATN_Minigame_AiTest_C_ReceiveBeginPlay_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.DelayDestroy
// (BlueprintCallable, BlueprintEvent)
void ATN_Minigame_AiTest_C::DelayDestroy()
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.DelayDestroy");

	ATN_Minigame_AiTest_C_DelayDestroy_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.MatchComplete
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// TEnumAsByte<GameResult_EGameResult> Result                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
void ATN_Minigame_AiTest_C::MatchComplete(TEnumAsByte<GameResult_EGameResult> Result)
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.MatchComplete");

	ATN_Minigame_AiTest_C_MatchComplete_Params params;
	params.Result = Result;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.ReceiveEndPlay
// (Event, Public, BlueprintEvent)
// Parameters:
// TEnumAsByte<Engine_EEndPlayReason> EndPlayReason                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
void ATN_Minigame_AiTest_C::ReceiveEndPlay(TEnumAsByte<Engine_EEndPlayReason> EndPlayReason)
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.ReceiveEndPlay");

	ATN_Minigame_AiTest_C_ReceiveEndPlay_Params params;
	params.EndPlayReason = EndPlayReason;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


// Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.ExecuteUbergraph_TN_Minigame_AiTest
// (Final)
// Parameters:
// int                            EntryPoint                     (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
void ATN_Minigame_AiTest_C::ExecuteUbergraph_TN_Minigame_AiTest(int EntryPoint)
{
	static auto fn = UObject::FindObject<UFunction>("Function TN_Minigame_AiTest.TN_Minigame_AiTest_C.ExecuteUbergraph_TN_Minigame_AiTest");

	ATN_Minigame_AiTest_C_ExecuteUbergraph_TN_Minigame_AiTest_Params params;
	params.EntryPoint = EntryPoint;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);
	fn->FunctionFlags = flags;

}


}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
